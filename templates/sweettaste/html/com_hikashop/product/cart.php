...
if(empty($this->rows)){
	...
	if(!empty($desc) || $desc == '0'){
        ...
        <p id="mini-cart" class="navbar-text navbar-right">
            <a href="<?php echo $url_checkout; ?>">
                <span class="glyphicon glyphicon-shopping-cart"></span>                    
                Корзина: <span class="badge">0</span>                    
            </a>
        </p>
<?php
	}
}else{ ?>
<div id="hikashop_cart" class="hikashop_cart navbar-right">
	<?php
...
<?php
	...
	if($this->params->get('small_cart')){
		...
		if($cart_type != 'wishlist'){
			?>
            <p id="mini-cart" class="navbar-text">
                <a href="<?php echo $url_checkout; ?>">
                    <span class="glyphicon glyphicon-shopping-cart"></span>                    
                    Корзина: <span class="badge"><?php echo $qty ?></span>                    
                </a>            
		    <?php
                if($this->params->get('show_cart_delete',1)){
                    ...
                    <button id="close-icon" title="Очистить корзину" type="button" class="close" onclick="window.location='<?php echo $delete.'return_url='; ?>'+window.btoa(window.location); return false;">
                        <span aria-hidden="true">&times;</span><span class="sr-only">Очистить корзину</span>
                        <span class="sr-only">Очистить корзину</span>
                    </button>
            <?php } ?>
            </p>
            <?php            
