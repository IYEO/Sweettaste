				...
				<?php
					...
					foreach($this->rows as $i => $row){
						...
						<tr class="row<?php echo $k; ?>">
							<?php if($this->params->get('show_cart_image')){ ?>
								<td data-title="<?php echo JText::_('CART_PRODUCT_IMAGE'); ?>" class="hikashop_cart_product_image_value">
									<?php if(!empty($row->images)){
										...
										$width = '300';//$this->config->get('thumbnail_x');
										$height = '300';//$this->config->get('thumbnail_y');
										...
										if(!$this->config->get('thumbnail')){
											...
											<?php
												...
												if($img->success) {
													echo '<img class="hikashop_product_checkout_cart_image center-block" title="'.$this->escape(@$image->file_description).'" alt="'.$this->escape(@$image->file_name).'" src="'.$img->url.'"/>';
												}
												?>
											</div>
									<?php }
							...
							<td data-title="<?php echo JText::_('CART_PRODUCT_NAME'); ?>" class="hikashop_cart_product_name_value">
								<p>
									...
									<?php if(@$defaultParams['link_to_product_page']){ ?><a class="hikashop_no_print" href="<?php echo hikashop_contentLink('product&task=show&cid='.$row->product_id.'&name='.$row->alias.$url_itemid,$row);?>" ><?php } ?>
										<?php echo 'Название: '.$row->product_name; ?><br />
										<span class="hikashop_product_code_checkout">Артикул: <?php echo $row->product_code; ?></span><br />
										<span><?php echo preg_replace('#<hr *id="system-readmore" */>.*#is','', $row->product_description); ?></span>		
							...
							<td data-title="<?php echo JText::_('PRODUCT_QUANTITY'); ?>" class="hikashop_cart_product_quantity_value text-center">
								<?php
								if(empty($this->disable_modifications)){
									if($row->product_quantity_layout == 'show_select' || (empty($row->product_quantity_layout) && $this->config->get('product_quantity_display', '') == 'show_select')){
										...
										?>
										<input id="hikashop_checkout_quantity_<?php echo $row->cart_product_id;?>" type="hidden" name="item[<?php echo $row->cart_product_id;?>]" class="hikashop_product_quantity_field" value="<?php echo $row->cart_product_quantity; ?>" onchange="var qty_field = document.getElementById('hikashop_checkout_quantity_<?php echo $row->cart_product_id;?>'); if (qty_field){<?php echo $input; ?>}; return true;" />
										<?php
									}
								?>
