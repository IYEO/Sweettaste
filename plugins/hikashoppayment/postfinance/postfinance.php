...
class plgHikashoppaymentPostfinance extends hikashopPaymentPlugin {
	...
	function onPaymentNotification(&$statuses){
		...
		if($this->payment_params->debug){
			$this->writeToLog("---------------------------------------START----------------------------------------<br/>");
			$this->writeToLog('$_REQUEST :'.print_r($_REQUEST,true).'<br/>');
			$this->writeToLog('date :'.print_r(getdate(),true).'<br/>');
		}
		ksort($result);
		$txtSha_tosecure ='';
		...
		if(empty($dbOrder)){
			if($this->payment_params->debug){
				$this->writeToLog("Could not load any order for your notification ".$vars['orderID']."NO ORDER ID <br/>");
			}
			return false;
		}
		...
		if($this->payment_params->debug){
			$this->writeToLog('result :'.print_r($result,true).'<br/>');
			$this->writeToLog('MYSHA : '.$txtSha.' THEIRCHA : '.$shasign.'<br/>');
			$this->writeToLog('sha_out :'.$this->payment_params->sha_out_phrase.'<br/>');
		}
